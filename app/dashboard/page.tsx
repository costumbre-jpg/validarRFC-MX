...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // aunque el gráfico filtrará visualmente lo que no sea de este mes.
  ...
  const handleValidationComplete = (options?: {
    ...
    // Refrescar desde BD eliminado para evitar parpadeo si la BD es lenta o falla
    // La actualización optimista anterior es suficiente para la sesión actual
        ...
        {userData?.subscription_status === "pro" || userData?.subscription_status === "business" ? (
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 max-md:p-4">
            <div className="flex items-center justify-between max-md:flex-col max-md:items-start max-md:gap-3">
              ...
              <Link
                href="/dashboard/historial"
                className="inline-flex items-center gap-2 max-md:gap-1.5 px-5 max-md:px-4 py-2.5 max-md:py-2 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors font-semibold text-sm max-md:text-xs shadow-sm hover:shadow-md w-full max-md:w-full justify-center max-md:justify-center"
              >
                Ver Historial
                <svg className="w-4 h-4 max-md:w-3.5 max-md:h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
