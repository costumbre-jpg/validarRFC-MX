  ...
  const handleCheckout = useCallback(async (planId: PlanId) => {
    ...
    try {
      ...
      if (testMode) {
        ...
        if (!resp.ok) {
          setErrorMessage(data.error || "No se pudo activar el plan");
          setProcessing(false);
      ...
      if (!response.ok) {
        setErrorMessage(data.error || "Error al crear sesión de checkout");
        setProcessing(false);
        setTimeout(() => setErrorMessage(null), 5000);
        return;
      }
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold text-white transition-all shadow-md hover:shadow-lg hover:scale-105 bg-brand-primary hover-bg-brand-secondary"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
